...
//    @PostMapping("/login")
    public String loginV2(@Valid @ModelAttribute LoginForm loginForm, BindingResult bindingResult, HttpServletResponse response) {
        ...
        sessionManager.createSession(loginMember, response); //세션 UUID만들고 쿠키 만들어서 쿠키 보냄
    ...
    /**
     * 세션에는 사실 많은 정보를 담으면 안된다.
     * 예제로 연습용이기 때문에 Member의 모든 정보를 세션에 담았지만
     * 실무에서는 MemberId 정도만 담자(그냥 최소한의 데이터만 담자)
     */
    @PostMapping("/login")
...
//    @PostMapping("/logout")
    ...
    public String logoutV3(HttpServletRequest request) {
        HttpSession session = request.getSession(false);//null일 경우(세션 없을 경우) 세션을 생성하지 않는다.
        if (session != null) {
            session.invalidate(); //안에 있는 데이터 모두 삭제됨
        }
        return "redirect:/";
    }
